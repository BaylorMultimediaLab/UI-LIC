...
def inference(model, x, f, outputpath, patch):
    ...
    recon_dir = os.path.join(outputpath, "reconstruction")
    bits_dir = os.path.join(outputpath, "bitstreams")
    os.makedirs(recon_dir, exist_ok=True)
    os.makedirs(bits_dir, exist_ok=True)

    imgPath = os.path.join(recon_dir, img_name)
    bit_path = os.path.join(bits_dir, f"{base_name}.pt")
    csvfile = os.path.join(outputpath, 'result.csv')
...
########original padding
    ...
    with open(csvfile, 'a+') as f_csv:
        row = [img_name, bpp * num_pixels, num_pixels, bpp, y_bpp, z_bpp,
...
def inference_entropy_estimation(model, x, f, outputpath, patch):
    ...
    with open(csvfile, 'a+') as f_csv:
        row = [img_name, bpp.item() * num_pixels, num_pixels, bpp.item(), y_bpp.item(), z_bpp.item(),
