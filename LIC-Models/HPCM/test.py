...
def test(args):
    ...
    #----- File Type Agnositc Change ------
    extensions = ['png', 'jpg', 'jpeg', 'bmp', 'PNG', 'JPG']
    images_list = []
    for ext in extensions:
        images_list.extend(glob.glob(os.path.join(args.dataset, f'*.{ext}')))

    # Sort them to ensure consistency across different OS environments
    images_list = sorted(images_list)

    print(f"DEBUG: Found {len(images_list)} images.")
    if len(images_list) == 0:
        print("CRITICAL: No images found. Check your dataset path and file extension.")
...
if __name__ == '__main__':
    ...
    # --- ADDED TO SAVE IMAGES ---
    parser.add_argument("--save_dir", type=str, default="./reconstructed_images", help="Folder to save output images")
