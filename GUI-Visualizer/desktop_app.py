...
class LICApp:
    ...
    def setup_ui(self):
        ...
        comp_controls_middle = ttk.Frame(comp_controls)
        comp_controls_middle.pack(fill=tk.X, pady=(0, 8))
        ...
        self.img_selector = ttk.Combobox(comp_controls_top, state="readonly", width=28, font=self.F_BASE)
        self.img_selector.pack(side=tk.LEFT, padx=(5, 0), fill=tk.X, expand=True)
        self.img_selector.bind("<<ComboboxSelected>>", self.update_comparison)

        ttk.Label(comp_controls_middle, text="Left Side:", font=self.F_BTN).pack(side=tk.LEFT)
        self.model_selector_left = ttk.Combobox(comp_controls_middle, state="readonly", width=16, font=self.F_BASE, values=["Ground Truth"])
        self.model_selector_left.set("Ground Truth")
        self.model_selector_left.pack(side=tk.LEFT, padx=(5, 12))
        self.model_selector_left.bind("<<ComboboxSelected>>", self.update_comparison)

        ttk.Label(comp_controls_middle, text="Right Side:", font=self.F_BTN).pack(side=tk.LEFT)
        self.model_selector_right = ttk.Combobox(comp_controls_middle, state="readonly", width=16, font=self.F_BASE, values=["Ground Truth"])
